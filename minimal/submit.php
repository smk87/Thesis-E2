...
while ($row = mysqli_fetch_array($r)) {
    ...
    if ($row['created']) {
        ...
        if ($row['bill_type'] == 'Hall Bill') {
            $sql = "UPDATE student SET latehall=latehall+1 WHERE std_id='$std_id'";
            if ($conn->query($sql) === true) {
                echo "Record updated successfully";
            } else {
                echo "Error updating record: " . $conn->error;
        ...
        if ($row['bill_type'] == 'Mess Bill') {
            $sql = "UPDATE student SET latemess=latemess+1 WHERE std_id='$std_id'";
            if ($conn->query($sql) === true) {
                echo "Record updated successfully";
            } else {
                echo "Error updating record: " . $conn->error;
